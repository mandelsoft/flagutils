...
func BytesBase64(f *pflag.FlagSet, name string, value []byte, usage string) *[]byte {
	p := new([]byte)
	BytesBase64VarP(f, p, name, "", value, usage)
	return p
}

func BytesBase64P(f *pflag.FlagSet, name, shorthand string, value []byte, usage string) *[]byte {
	p := new([]byte)
	BytesBase64VarP(f, p, name, shorthand, value, usage)
	return p
}
