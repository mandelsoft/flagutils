...
func setUpSCSSFlagSet(scssp *MySSM) *FlagSet {
	...
	pflags.StringColonStringSliceVar(f, scssp, "scss", nil, "Command separated lscst!")
	return f
}

func setUpSCSSFlagSetWithDefault(scssp *MySSM) *FlagSet {
	f := NewFlagSet("test", ContinueOnError)
	pflags.StringColonStringSliceVar(f, scssp, "scss", map[string][]string{"da": {"1", "2", "3"}, "db": {"2"}, "de": {"5,6", "7"}}, "Command separated lscsst!")
	return f
}
