...
func setUpS2SSFlagSet(s2ssp *MySSM) *FlagSet {
	...
	pflags.StringToStringSliceVar(f, s2ssp, "s2ss", map[string][]string{}, "Command separated ls2st!")
	return f
}

func setUpS2SSFlagSetWithDefault(s2ssp *MySSM) *FlagSet {
	f := NewFlagSet("test", ContinueOnError)
	pflags.StringToStringSliceVar(f, s2ssp, "s2ss", map[string][]string{"da": {"1", "2", "3"}, "db": {"2"}, "de": {"5,6", "7"}}, "Command separated ls2sst!")
	return f
}
