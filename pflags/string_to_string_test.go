...
func setUpS2SFlagSet(s2sp *MyMap) *FlagSet {
	...
	pflags.StringToStringVar(f, s2sp, "s2s", map[string]string{}, "Command separated ls2st!")
	return f
}

func setUpS2SFlagSetWithDefault(s2sp *MyMap) *FlagSet {
	f := NewFlagSet("test", ContinueOnError)
	pflags.StringToStringVar(f, s2sp, "s2s", map[string]string{"da": "1", "db": "2", "de": "5,6"}, "Command separated ls2st!")
	return f
}
